...
\SI{150}{MeV}; this gives $\mathrm{RSP}\approx1.0$ (soft tissue), $1.03$ (brain),
$1.6$ (cortical bone). Equation~\eqref{eq:rsp} is the principled form of the
...
bone, \SI{43}{mm} brain),
\[
\mathrm{WEPL}=4(1.0)+8(1.6)+43(1.03)\approx\SI{61}{mm},
\]
i.e.\ the bone pushes the radiological depth ${\sim}\SI{6}{mm}$ beyond the
geometric one; the distal target maps similarly to ${\sim}\SI{113}{mm}$.
...
\end{enumerate}
This is an opt-in mode of \texttt{field\_design/sobp.py} (a \texttt{-{}-regions}
flag pointing at a run's \texttt{phantom\_regions.csv}); without it, the
homogeneous $\rho_{\mathrm{rel}}$ path of the standard cylinder scenario is
untouched.
...
\texttt{-{}-regions} & \texttt{phantom\_regions.csv} for WEPL design (Sec.~\ref{sec:wepl}) & none (homogeneous) \\
...
The no-bone control (\texttt{uniform\_head}, same envelope, brain throughout)
isolates the skull's effect on both.
