...
energies combined so the dose is flat across the whole target volume. The energy layers and their weights are designed separately; the method
...
\paragraph{Cylinder (the standard).} A single homogeneous cylinder with its axis
along the beam. This is the default for the reference study: a uniform
medium gives a clean ground truth, the simplest case before tissue heterogeneity
and biological washout enter the picture. The standard material is brain tissue (\texttt{G4\_BRAIN\_ICRP}) in a
...
tissue by default. It keeps the homogeneous, clean-ground-truth character of the
