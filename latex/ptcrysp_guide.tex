...
treated. \texttt{ptcryspg4} implements a homogeneous brain-tissue cylinder, a
homogeneous head filled with brain tissue, and a MIRD head built from three
materials (scalp, bone and brain tissue).
\end{itemize}
...
\label{sec:phantom}
The three phantoms share the same beam axis ($z$) and the same target-box dose
...
material & phantom material & \file{G4\_BRAIN\_ICRP} \\
\bottomrule
\end{tabular}
\end{center}
...
primary. A uniform fluence over a disk (\file{/stageA/beam/disk}) matched to the
...
and the target window enter the design, the cylinder, \file{mird\_head} and
\file{uniform\_head} fields are different tables. Each design is written per field,
...
\file{analysis\_transport/make\_figures.py} renders it and reports the plateau
flatness. Acceptance is by \textbf{R80}, the distal \SI{80}{\percent}-dose depth
...
deterministic budget; the Poisson realisations and the sampling of annihilation
points are done by the downstream detector study, so the same budget and seed
yield an identical source for every detector.
...
\file{beam\_energy\_MeV} & float & nominal proton energy (SOBP: fallback single energy) \\
\file{beam\_sigma\_mm} & float & Gaussian beam $\sigma$ at entrance (pencil mode) \\
\file{geometry} & string & \file{cylinder} \textbar{} \file{uniform\_head} \textbar{} \file{mird\_head} \\
...
\file{t\_irr\_s}, \file{t\_del\_s}, \file{t\_meas\_s}. The standard set is
\file{inroom} (baseline, $\tdel=\SI{120}{s}$), \file{fast} ($\tdel=\SI{60}{s}$),
and \file{offline} ($\tdel=\SI{300}{s}$, $\tmeas=\SI{1800}{s}$); shorter delay
keeps more short-lived $\Ox{15}$.
...
\file{figures/} holds the realised depth-dose, the validation dashboard, and the
activity curves.
...
\subsection{Worked example: \file{head\_sobp\_1e7}}
\label{sec:example}
...
\file{common/SCHEMA.md}. A frozen run is checked with
\file{analysis\_transport/check\_run.py} and against the Parodi 2008 yields with
\file{analysis\_transport/parodi\_cross\_check.py}.
