# AGENTS.md — Proton-therapy PET source generation (Stage A + handoff)

Orientation for any coding-agent session on this repo.
...
Terminal one-liners, REPL sessions, temporary scripts and scratch notebooks may
be used only for diagnostics; promote any result used in the work to a tracked
script and regenerate it there before reporting or publishing it.
...
This file records the implementation decisions, parameters, and
...
`budget_gen.py`). The standard run (`uniform_headep_sobp_1e8`, 10⁸ protons) is
frozen in the `ptcrysp-scenarios` repo. The source side is complete; the detector
study is a separate downstream repo.
