...
the user guide (\file{simulate\_pt\_pet.tex}) and the decay bookkeeping in
\file{handoff.tex}; this note is the consumer-facing interface contract.
\end{abstract}
...
\begin{description}[leftmargin=1.6em]
  ...
  \item[\emph{through what}] the phantom medium the \SI{511}{keV} photons
    traverse (\file{phantom\_material\_<name>.csv}).
\end{description}
...
\subsection{\file{phantom\_material\_<name>.csv} --- the medium}
...
phantom and needs the attenuation coefficient $\mu$ for reconstruction
attenuation correction. \file{<name>} is the \file{phantom\_material},
lower-cased (e.g.\ \file{g4\_brain\_icrp}). The composition file has one row per
element (\file{element}, \file{Z}, \file{A\_g\_mol}, \file{mass\_fraction}); the
companion \file{*\_meta.csv} carries \file{density\_g\_cm3},
\file{mean\_excitation\_eV}, $\mu/\rho$ (\file{mu\_rho\_cm2\_g}) and $\mu$
(\file{mu\_cm\_inv}) at \file{energy\_keV}$=511$, and \file{mean\_free\_path\_cm}.
Compositions are the authoritative Geant4 NIST definitions; $\mu$ is
Compton/Klein--Nishina (coherent $+$ photoelectric, $\sim$1--\SI{2}{\percent} at
\SI{511}{keV}, are not included). This is valid only for the \emph{homogeneous}
phantom of the standard scenarios; a heterogeneous (voxelized) phantom would
instead require a voxel attenuation map, which is deferred.
...
\begin{enumerate}[leftmargin=1.8em]
  ...
  \item Transport the photons through the phantom defined by
    \file{phantom\_material\_<name>.csv} and the geometry of Sec.~\ref{sec:frame};
    use the same medium for reconstruction attenuation correction.
\end{enumerate}
...
$\sigma(\text{range})$, is the figure of merit (\file{handoff.tex}).
